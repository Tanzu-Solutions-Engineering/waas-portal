/*
 * Kubernetes
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: v1.21.1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */


package com.vmware.tanzu.learningcenter.models;

import java.util.Objects;
import java.util.Arrays;
import com.google.gson.TypeAdapter;
import com.google.gson.annotations.JsonAdapter;
import com.google.gson.annotations.SerializedName;
import com.google.gson.stream.JsonReader;
import com.google.gson.stream.JsonWriter;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import java.io.IOException;
import java.util.ArrayList;
import java.util.List;

/**
 * V1beta1SystemProfileSpecEnvironmentSecrets
 */
@jakarta.annotation.Generated(value = "org.openapitools.codegen.languages.JavaClientCodegen", date = "2022-03-07T20:08:06.732Z[Etc/UTC]")
public class V1beta1SystemProfileSpecEnvironmentSecrets {
  public static final String SERIALIZED_NAME_PULL = "pull";
  @SerializedName(SERIALIZED_NAME_PULL)
  private List<String> pull = null;


  public V1beta1SystemProfileSpecEnvironmentSecrets pull(List<String> pull) {
    
    this.pull = pull;
    return this;
  }

  public V1beta1SystemProfileSpecEnvironmentSecrets addPullItem(String pullItem) {
    if (this.pull == null) {
      this.pull = new ArrayList<>();
    }
    this.pull.add(pullItem);
    return this;
  }

   /**
   * Get pull
   * @return pull
  **/
  @jakarta.annotation.Nullable
  @ApiModelProperty(value = "")

  public List<String> getPull() {
    return pull;
  }


  public void setPull(List<String> pull) {
    this.pull = pull;
  }


  @Override
  public boolean equals(Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    V1beta1SystemProfileSpecEnvironmentSecrets v1beta1SystemProfileSpecEnvironmentSecrets = (V1beta1SystemProfileSpecEnvironmentSecrets) o;
    return Objects.equals(this.pull, v1beta1SystemProfileSpecEnvironmentSecrets.pull);
  }

  @Override
  public int hashCode() {
    return Objects.hash(pull);
  }


  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class V1beta1SystemProfileSpecEnvironmentSecrets {\n");
    sb.append("    pull: ").append(toIndentedString(pull)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces
   * (except the first line).
   */
  private String toIndentedString(Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }

}

